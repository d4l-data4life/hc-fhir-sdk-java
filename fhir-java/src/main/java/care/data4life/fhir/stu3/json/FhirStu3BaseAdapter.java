...
public class FhirStu3BaseAdapter extends JsonAdapter<FhirStu3Base> {
    ...
    @Nullable
    @Override
    public FhirStu3Base fromJson(JsonReader reader) throws IOException {
        return fromJson(new BigDecimalDelegateJsonReader(reader));
    }

    @Nullable
    @SuppressWarnings("unchecked")
    FhirStu3Base fromJson(BigDecimalDelegateJsonReader reader) throws IOException {
        ...
        if (jsonObject instanceof Map) {
            ...
            if (resourceType instanceof String) {
                try {
                    Class<?> clazz = Class.forName(FhirStu3Base.class.getPackage().getName() + "." + resourceType);
                    ...
                    if (jsonAdapter != null) {
                        if (jsonMap.containsKey(JSON_KEY_CONTAINED)) {
                            ...
                            return (FhirStu3Base) jsonAdapter.fromJsonValue(jsonObject);
                        }
                    }
            ...
            for (Object containedObject : jsonContainedList) {
                if (containedObject instanceof Map) {
                    ...
                    if (containedResourceType instanceof String) {
                        try {
                            Class<?> containedClazz = Class.forName(FhirStu3Base.class.getPackage().getName() + "." + containedResourceType);
    ...
    @Override
    @SuppressWarnings("unchecked")
    public void toJson(JsonWriter writer, @Nullable FhirStu3Base value) throws IOException {
        if (value == null) {
            return;
        }
