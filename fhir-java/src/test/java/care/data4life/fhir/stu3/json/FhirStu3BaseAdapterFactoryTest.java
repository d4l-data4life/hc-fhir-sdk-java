...
public class FhirStu3BaseAdapterFactoryTest {

    private Moshi mockMoshi = mock(Moshi.class);


    // SUT
    private FhirStu3BaseAdapterFactory factory;


    @Before
    public void setup() {
        factory = new FhirStu3BaseAdapterFactory();
    }

    @Test
    public void create_shouldReturnFhirStu3BaseAdapter_whenTypeFhirStu3BaseClass() {
        // Given
        Type type = FhirStu3Base.class;

        // When
        JsonAdapter adapter = factory.create(type, null, mockMoshi);

        // Then
        assertThat(adapter, instanceOf(FhirStu3BaseAdapter.class));
    }
