...
public enum CodeSystemObservationRelationshipType {
    ...
    /**
     * The target resource (Observation or QuestionnaireResponse) is part of the information from
     * which this observation value is derived. (e.g. calculated anion gap, Apgar score)  NOTE:
     * "derived-from" is the only logical choice when referencing QuestionnaireResponse.
     */
    @Json(name = "derived-from")
    DERIVED_FROM,

    /**
     * This observation follows the target observation (e.g. timed tests such as Glucose Tolerance
     * Test).
     */
    @Json(name = "sequel-to")
    SEQUEL_TO,
     ...
     * observation) with the semantics of the source observation (e.g. a hemolysis measure target
     * from a plasma potassium measure, which has no value).
     */
