...
/**
 * Starter set of user interface control/display mechanisms that might be used when rendering an item in a
 * questionnaire.
 *
...
public enum CodeSystemQuestionnaireItemUIControlCodes {
    ...
    /**
     * Text is displayed immediately below or within the answer-entry area of the containing question item (typically as
     * a guide for what to enter)
     */
    @Json(name = "prompt")
    PROMPT,

    /**
     * Text is displayed adjacent (horizontally or vertically) to the answer space for the parent question, typically to
     ...
     * action to request 'help' for a question, group or the questionnaire as a whole (depending what the text is nested
     * within)
     */
    @Json(name = "help")
    HELP,
    ...
    /**
     * A control which provides a list of potential matches based on text entered into a control.  Used for large choice
     * sets where text-matching is an appropriate discovery mechanism.
     */
     ...
     * choice.  Multiple selections may be possible.
     */
    @Json(name = "check-box")
    CHECK_BOX,
    ...
    /**
     * A control where choices are listed with a button beside them.  The button can be toggled to select or de-select a
     * given choice.  Selecting one item deselects all others.
     */
