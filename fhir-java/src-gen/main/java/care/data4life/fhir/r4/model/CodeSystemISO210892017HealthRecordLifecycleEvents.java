...
/**
 * Attached is vocabulary for the 27 record lifecycle events, as per ISO TS 21089-2017, Health Informatics - Trusted
 * End-to-End Information Flows, Section 3, Terms and Definitions (2017, at ISO Central Secretariat, passed ballot and
 * ready for publication).  This will also be included in the FHIR EHR Record Lifecycle Event Implementation Guide,
 * balloted and (to be) published with FHIR STU-3.
 *
...
public enum CodeSystemISO210892017HealthRecordLifecycleEvents {
    ...
    /**
     * Occurs when an agent causes the system to capture the agent’s digital signature (or equivalent indication) during
     * formal validation of record entry content.
     */
    @Json(name = "attest")
    ATTEST,
    ...
    /**
     * Occurs when an agent causes the system to combine or join content from two or more record entries, resulting in a
     * single logical record entry.
     */
    @Json(name = "merge")
    MERGE,
    ...
    /**
     * Occurs when an agent causes the system to recreate or restore full status to record entries previously deleted or
     * deprecated.
     */
    ...
    /**
     * Occurs when an agent causes the system to change the form, language or code system used to represent record entry
     * content.
     */
    @Json(name = "transform")
    TRANSFORM,
