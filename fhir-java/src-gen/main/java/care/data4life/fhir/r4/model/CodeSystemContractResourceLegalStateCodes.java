 ...
 *
 * @see <a href="http://hl7.org/fhir/contract-legalstate">ContractResourceLegalStateCodes</a>
 */
public enum CodeSystemContractResourceLegalStateCodes {
     ...
     * state for normal completion of contracting activity.  Precedence Order = 6. Comparable FHIR and v.3 status codes:
     * accepted; completed.
     */
    @Json(name = "executed")
    EXECUTED,
     ...
     * insurer declines to agree to the signed application because the life insurer has evidence that the applicant, who
     * asserted to being younger or a non-smoker to get a lower premium rate - but offers instead to agree to a higher
     * premium based on the applicants actual age or smoking status. Usage: Optional contract activity between
    ...
    /**
     * Contract is a proposal by either the Grantor or the Grantee. Aka - A Contract hard copy or electronic 'template',
     * 'form' or 'application'. E.g., health insurance application; consent directive form. Usage: Beginning of contract
     * negotiation, which may have been completed as a precondition because used for 0..* contracts. Precedence Order =
     * 2. Comparable FHIR and v.3 status codes: requested; new.
     */
    @Json(name = "offered")
    OFFERED,
     ...
     * completed as a precondition because used for 0..* contracts. Precedence Order = 1. Comparable FHIR and v.3 status
     * codes: proposed; intended.
     */
    @Json(name = "policy")
    POLICY,

    /**
     * Execution of the Contract is not completed because either or both the Grantor and Grantee decline to accept some
